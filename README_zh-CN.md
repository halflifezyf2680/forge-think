...
**注**：forge-think server 严格遵循 MCP 模式，不调用外部 LLM API，仅负责编排、状态管理和持久化。
...
很多人会问：“我直接写个包含所有步骤的长 Prompt（或者静态 Skill）喂给大模型不行吗？为什么要搞个需要配置、有门槛的 MCP Server 这么麻烦？”

核心原因在于**工程化地控制推理深度**。

大家在实际使用中很容易发现：给大模型一次性塞入海量 Prompt 期望它在一次输出中搞定，和**强制打断它的输出流程，迫使它分成几次迭代交互**，最终得到的质量和深度是截然不同的。

1. **“静态建议”与“强制状态机”**：如果只是一份静态的长篇 Skill，对大模型来说仅仅是“建议”。面对复杂任务，模型极易偷懒、跳过中间的细致分析直接“抖机灵”给出最终结论，导致思考退化。而 forge-think 是一个**强制状态机**，模型必须通过调用 `forge_step` 等工具交互才能拿到下一步的授权，从物理层面掐断了它“一步到位”的捷径。
2. **打破“伪反思”幻觉，强制“看见”输出**：如果在单一 Prompt 中要求“先给出方案，再客观评价你的方案”，模型实际上只是在做字面上的文本接龙。它所谓的“评价”只是受生成前指令影响的连续输出，和它“有没有真切地审视自己最终完整的回答”毫无关系，极易沦为走过场。而 forge-think 通过强制打断（如 `deliberate` 模式），让上一轮的回答必须先生成并“定型”（commit），下一轮的审查角色才能在物理层面真正作为上下文“看见”这段文本，从而产生深刻、真实的评估。
3. **收敛注意力的深度**：将大任务强制拆分为多回合（Multi-turn）交互，保证了每一步的模型注意力（Attention）都极度集中，不再被几十条并发规则分散，这才是拔高模型推理上限的工程方法。
4. **状态持久化与防污染**：将动辄千字的 SOP 留在服务端按步调度，避免了长串文本给上下文（Context Window）带来的无谓污染；同时每一步的落盘追踪，让超长任务不仅能断点续传，还能精准对齐复盘。
